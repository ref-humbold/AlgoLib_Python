...
class ConvexHullTest(unittest.TestCase):
    ...
    def test__find_convex_hull__when_one_point__then_empty(self):
        # when
        result = find_convex_hull([Point2D(3.0, 2.0)])
        # then
        self.assertEqual(0, len(result))

    def test__find_convex_hull__when_two_points__then_empty(self):
        # when
        result = find_convex_hull([Point2D(2.0, 3.0), Point2D(3.0, 2.0)])
        # then
        self.assertEqual(0, len(result))

    def test__find_convex_hull__when_three_points__then_these_points_are_hull(self):
        # given
    ...
    def test__find_convex_hull__then_points_in_hull(self):
        # when
